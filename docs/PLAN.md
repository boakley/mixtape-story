...
- **Apple Music API swap — SHIPPED 2026-05-21** (no longer deferred). Trigger fired: a user asked "is Spotify supported?" on "A Horse With No Name." Apple Music API replaced iTunes Search as the resolver, `songs.links_by_platform` now stores Odesli's per-platform links, and a visitor "Listen with" chip routes each song to the preferred service. Twist discovered mid-build: Spotify's API is Premium-gated *and* Odesli stopped surfacing Spotify/YouTube entirely, so those route to public search URLs rather than direct links. Full write-up incl. what diverged from plan in [`PHASE-2-resolver-and-pref.md`](PHASE-2-resolver-and-pref.md).
- **Onboarding nudges for sign-ups who never added songs (considered 2026-05-26).** Real case that prompted this: Bryan showed Dan the app, Dan was excited, signed up, then forgot — the empty-mixtape sits there indefinitely and the excitement spike is wasted. Shape: a **three-email gentle nudge sequence at day 2 / day 7 / day 21, then silence** — anything more reads as desperate at writing-group scale. Trigger query: `auth.users` rows older than threshold with zero rows in `mixtape_songs`. Mechanism is a `pg_cron`-scheduled Supabase Edge Function calling Resend (same pattern as the resolve queue), idempotent via `onboard_nudge_N_at` columns on the user row so re-runs don't double-send. **Personalization is the load-bearing decision**: when the signup came via a group invite, the email mentions the group explicitly ("you joined Writing Meditation but haven't added songs yet") — that's the strongest reactivation hook because the social context is *why* they signed up. Without groups the copy falls back to a generic "you started a mixtape on mixtapestory.com…" and is materially weaker — which is why **the build trigger is "after Phase 3a lands"**, when group context becomes available, OR when a third signed-up-but-empty user surfaces, whichever comes first. Non-negotiables: one-click unsubscribe link in every email (even at writing-group scale), and copy that matches the rest of the product — no exclamation marks, no marketing voice. Open question for build time: how to handle users who signed up to *read* a friend's tape and never intended to create their own — three emails over three weeks is probably gentle enough that they won't churn, but a "just here to read" signup option would solve it cleanly if it ever becomes an issue.
