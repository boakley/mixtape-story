...
- **Apple Music API swap (planned, 2026-05-18).** Trigger condition fired: a writing-group user asked "is Spotify supported?" on "A Horse With No Name." Root cause is ISRC fragmentation in old catalog, not Spotify support — Odesli can't bridge the ISRC iTunes hands it to any of Spotify's many ISRCs for the same recording. Apple Music API as primary resolver source improves the bridging because Apple's catalog graph is what Odesli aligns to most reliably. Bundled with the long-pending visitor "Listen with" preference (per-song href routing based on a cookie). Full plan in [`PHASE-2-resolver-and-pref.md`](PHASE-2-resolver-and-pref.md) — ~7-9 hours of work across 6 independently shippable commits.
